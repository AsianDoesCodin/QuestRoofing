...
const cacheVersion = "2026-06-08-lighthouse-02";
...
const estimatePage = (prefix = "") => link(prefix, "request-estimate/index.html");
...
const issueDiagnosticCard = ([heading, text], index) => `<article class="issue-card issue-card--diagnostic"><span class="issue-card-icon" aria-hidden="true">${String(index + 1).padStart(2, "0")}</span><div><h3>${heading}</h3><p>${text}</p><a href="${estimatePage()}">Request inspection <span aria-hidden="true">&rarr;</span></a></div></article>`;
  ...
  return `<header class="site-header" id="top">
    ...
    <div class="header-main">
      <div class="header-brand-panel">
        <a class="brand-lockup" href="${prefix ? `${prefix}/index.html` : "index.html"}"><img class="brand-wordmark" src="${versionedAsset(prefix, logoPath)}" alt="Quest Roofing" width="1184" height="624"><span class="brand-location">Queen Creek, Arizona</span></a><div class="header-proof-cards" aria-label="Quest Roofing credentials and contact"><a class="header-proof-card" href="${rocUrl}" target="_blank" rel="noopener"><span>License</span><strong>AZ ROC #355136</strong></a><a class="header-proof-card" href="${gafUrl}" target="_blank" rel="noopener"><span>Credential</span><strong>GAF Certified</strong></a><a class="header-proof-card" href="${estimatePage(prefix)}"><span>Estimate</span><strong>Free written estimates</strong><em>Licensed, bonded, insured</em></a><a class="header-proof-card header-proof-card-phone" href="${phoneHref}"><span>Call us</span><strong>${phone}</strong></a></div>
      </div>
...
function inlineCta(prefix) {
  return `<div class="inline-cta"><strong>Need a documented roof answer?</strong><a class="button button-primary" href="${estimatePage(prefix)}">Schedule Free Inspection</a><a class="button button-secondary" href="${phoneHref}">Call ${phone}</a></div>`;
}
...
function homepage() {
  ...
  const gallery = [["protection", "Property protection"], ["staged", "Tile staged for underlayment"], ["battens", "Battens and underlayment"], ["flashing", "Flashing detail"], ["pipe", "Pipe flashing detail"], ["shingle", "Finished shingle surface"], ["tileFinished", "Finished tile roof slope"]];
  ...
  const body = `<main id="main">
    ...
    <section class="section-band section-area" id="service-area"><div class="container area-layout"><div><p class="eyebrow">Service area</p><h2>Queen Creek-based. Serving the Greater Phoenix area.</h2><p>Quest is based in Queen Creek and serves nearby Phoenix-area communities with roof inspections, repairs, replacements, and written estimates.</p></div><div class="area-panel"><div class="map-card service-area-summary" aria-label="Quest Roofing Greater Phoenix service area"><span>Service radius</span><strong>Greater Phoenix</strong><em>Queen Creek-based roof inspections, repairs, replacements, and written estimates for nearby communities.</em></div><p class="map-note"><strong>Service radius:</strong> approximate Greater Phoenix coverage area for nearby inspections and estimates.</p><div class="city-pills">${cities.map((city) => `<a href="roofing-${city.slug}-az/index.html">${city.name}</a>`).join("")}</div></div></div></section>
  ...
  return layout({
    ...
    schema,
    bodyClass: "home",
    extraHead: `<link rel="preload" as="image" href="assets/quest-roofing-real/hero-roof-replacement-drone.webp" type="image/webp" fetchpriority="high" media="(min-width: 68.01rem)">`
  });
}
...
function galleryPage(path = "gallery/") {
  ...
  const body = `<main id="main" class="subpage-main"><section class="subpage-hero support-hero"><div class="container subpage-hero-grid"><div><p class="eyebrow">Gallery</p><h1>${h1}</h1><p>${meta}</p><div class="hero-actions"><a class="button button-primary" href="${home(prefix, "estimate")}">Schedule Free Inspection</a><a class="button button-secondary button-on-dark" href="${phoneHref}">Call ${phone}</a></div></div>${mediaFigure("subpage-photo", "drone", prefix, " loading=\"eager\" decoding=\"async\"", "Project documentation and roof detail photos.")}</div></section>${section("Browse project documentation.", "", `<div class="gallery-wall">${photos.map(([photo, caption], index) => `<a class="image-card image-card--overlay ${index % 5 === 0 ? "wide" : ""}" href="${asset(prefix, images[photo][0])}" target="_blank" rel="noopener">${mediaFrame(photo, prefix, " loading=\"lazy\" decoding=\"async\"")}<span class="caption">${caption}</span></a>`).join("")}</div>`, "section-gallery-page")}${inlineCta(prefix)}</main>`;
...
:root {
  ...
  --color-roof-orange: #B94216;
  --color-burnt-clay: #9D3514;
...
const serviceLinkCardCss = `.service-link-grid a{display:grid;align-content:start;gap:var(--space-2)}.service-link-grid strong,.service-link-grid span{display:block}`;
...
const accessibilityColorCss = `.hero .eyebrow,.subpage-hero .eyebrow,.section-gallery-teaser .eyebrow,.section-process .eyebrow,.final-cta .eyebrow,.site-footer .footer-contact span{color:var(--color-on-dark-accent)}.site-footer .footer-link-grid a:hover,.site-footer .footer-link-grid a:focus-visible,.site-footer .footer-bottom a:hover,.site-footer .footer-bottom a:focus-visible{color:var(--color-on-dark-accent)}`;
const headerStabilizeCss = `
/* Header proof-card and dropdown layout */
@media (min-width:76.01rem){
  .site-header,.header-main,.header-nav-panel{overflow:visible}
  .header-main{display:block;min-height:0;padding:0 clamp(var(--space-4),5vw,var(--space-20));background:var(--color-warm-off-white)}
  .header-brand-panel{display:flex;align-items:center;justify-content:space-between;gap:clamp(1.25rem,3vw,3rem);min-height:5.35rem;padding:0;background:transparent;clip-path:none;box-shadow:none;overflow:visible}
  .header-brand-panel::before,.header-brand-panel::after{display:none}
  .brand-lockup{width:auto;min-height:4.75rem;padding:0}
  .brand-wordmark{width:clamp(14.5rem,19vw,20rem);height:auto;max-height:4.6rem}
  .brand-location{color:var(--color-deep-navy)}
  .header-proof-cards{display:grid;grid-template-columns:repeat(4,minmax(0,auto));align-items:stretch;justify-content:end;gap:.65rem;min-width:0;margin-left:auto}
  .header-proof-card{display:grid;align-content:center;min-width:8.25rem;min-height:3.8rem;border:var(--rule) solid color-mix(in srgb,var(--color-line),transparent 18%);border-radius:.65rem;background:linear-gradient(180deg,var(--color-surface),color-mix(in srgb,var(--color-quest-blue),var(--color-warm-off-white) 90%));color:var(--color-deep-navy);padding:.55rem .7rem;box-shadow:inset 0 1px 0 color-mix(in srgb,var(--color-white),transparent 10%),0 10px 22px color-mix(in srgb,var(--color-deep-navy),transparent 92%);transition:transform var(--dur-fast) var(--ease-out),border-color var(--dur-fast) var(--ease-out),box-shadow var(--dur-fast) var(--ease-out)}
  .header-proof-card:hover,.header-proof-card:focus-visible{transform:translateY(-.08rem);border-color:color-mix(in srgb,var(--color-quest-blue),transparent 46%);box-shadow:inset 0 1px 0 color-mix(in srgb,var(--color-white),transparent 10%),0 14px 28px color-mix(in srgb,var(--color-deep-navy),transparent 88%)}
  .header-proof-card span{color:var(--color-accent-strong);font-size:.68rem;font-weight:900;letter-spacing:.06em;line-height:1.1;text-transform:uppercase}
  .header-proof-card strong{display:block;margin-top:.18rem;color:var(--color-deep-navy);font-size:clamp(.82rem,.8vw,.94rem);font-weight:900;line-height:1.15;white-space:nowrap}
  .header-proof-card em{display:block;margin-top:.12rem;color:var(--color-ink-soft);font-size:.72rem;font-style:normal;font-weight:750;line-height:1.1;white-space:nowrap}
  .header-proof-card-phone{border-color:color-mix(in srgb,var(--color-deep-navy),transparent 72%);background:var(--color-deep-navy);color:var(--color-white)}
  .header-proof-card-phone span{color:var(--color-on-dark-accent)}
  .header-proof-card-phone strong{color:var(--color-white)}
  .header-nav-panel{display:block;min-width:0;padding:.85rem 0 0;border-top:var(--rule) solid color-mix(in srgb,var(--color-line),transparent 12%);background:transparent}
  .header-nav-panel::before{display:none}
  .header-nav-panel .site-nav{display:flex;align-items:center;justify-content:flex-start;min-height:3.65rem;width:100%;gap:0;padding:0;font-size:clamp(.95rem,1vw,1.08rem);font-weight:750;overflow:visible}
  .header-nav-panel .site-nav > a,.header-nav-panel .site-nav > .nav-dropdown > .nav-parent{position:relative;min-height:3.25rem;border:0;border-radius:.75rem;background:transparent;color:var(--color-deep-navy);box-shadow:none;padding:.85rem clamp(.9rem,1.45vw,1.35rem);transition:transform var(--dur-fast) var(--ease-out),background-color var(--dur-fast) var(--ease-out),color var(--dur-fast) var(--ease-out)}
  .header-nav-panel .site-nav > a:not(.nav-cta):hover,.header-nav-panel .site-nav > a:not(.nav-cta):focus-visible,.header-nav-panel .site-nav > .nav-dropdown > .nav-parent:hover,.header-nav-panel .site-nav > .nav-dropdown:focus-within > .nav-parent{transform:translateY(-.06rem);background:color-mix(in srgb,var(--color-quest-blue),transparent 90%);color:var(--color-strong-navy)}
  .header-nav-panel .site-nav .nav-cta{min-height:3.35rem;margin-left:auto;padding:.9rem clamp(1.25rem,2vw,2rem);border:0;border-radius:.15rem;background:var(--color-accent);color:var(--color-white);font-size:clamp(.94rem,1vw,1.04rem);font-weight:900;box-shadow:inset 0 -3px 0 color-mix(in srgb,var(--color-deep-navy),transparent 82%),0 12px 24px color-mix(in srgb,var(--color-accent),transparent 78%)}
  .header-nav-panel .site-nav .nav-cta::after{content:">";margin-left:.5rem;font-size:1.2em;line-height:1}
  .nav-dropdown{position:relative;display:inline-flex;align-items:center}
  .header-nav-panel .site-nav .nav-parent{gap:.35rem}
  .header-nav-panel .site-nav .nav-parent::after{content:"";position:static;display:inline-block;width:.45rem;height:.45rem;margin-left:.2rem;border-right:2px solid currentColor;border-bottom:2px solid currentColor;transform:rotate(45deg) translateY(-.12rem);transition:transform var(--dur-fast) var(--ease-out)}
  .header-nav-panel .site-nav .nav-dropdown:hover .nav-parent::after,.header-nav-panel .site-nav .nav-dropdown:focus-within .nav-parent::after{transform:rotate(225deg) translateY(-.08rem)}
  .nav-menu{position:absolute;top:100%;left:50%;z-index:70;display:none;min-width:18.5rem;max-width:min(22rem,calc(100vw - 2rem));padding:1.4rem .85rem .85rem;border:var(--rule) solid color-mix(in srgb,var(--color-quest-blue),transparent 34%);border-radius:var(--radius-card);background:var(--color-deep-navy);box-shadow:var(--shadow-panel);transform:translate(-50%,0)}
  .nav-dropdown:hover .nav-menu,.nav-dropdown:focus-within .nav-menu{display:grid}
  .header-nav-panel .site-nav .nav-menu a{display:flex;justify-content:flex-start;width:100%;min-height:2.75rem;border-radius:var(--radius-md);color:var(--color-on-dark-soft);padding:.65rem .85rem;font-size:var(--text-sm);font-weight:900;text-transform:uppercase;letter-spacing:.02em}
  .header-nav-panel .site-nav .nav-menu a:hover,.header-nav-panel .site-nav .nav-menu a:focus-visible{background:color-mix(in srgb,var(--color-white),transparent 92%);color:var(--color-white)}
}
@media (max-width:76rem){
  .top-bar,.header-proof-cards{display:none}
  .site-header{z-index:100}
  .header-main{position:relative;display:flex;align-items:center;min-height:5rem;padding:.55rem var(--space-3);overflow:visible;background:var(--color-warm-off-white)}
  .header-brand-panel{flex:1 1 auto;align-items:center;min-width:0;padding:0;background:transparent;clip-path:none;box-shadow:none;overflow:visible}
  .header-brand-panel::before,.header-brand-panel::after{display:none}
  .brand-lockup{min-height:3.9rem;padding:.35rem 0}
  .brand-wordmark{width:auto;height:clamp(3rem,8vw,3.65rem);max-width:min(100%,14rem)}
  .brand-location{font-size:.68rem;color:var(--color-ink-soft)}
  .header-nav-panel{position:static;display:flex;align-items:center;justify-content:flex-end;flex:0 0 auto;gap:.45rem;min-width:0;padding-left:var(--space-2);background:transparent}
  .header-nav-panel .site-nav{position:fixed;inset:5rem 0 auto 0;z-index:99;display:grid;grid-template-columns:1fr;gap:.45rem;width:100vw;max-height:calc(100dvh - 5rem);overflow-y:auto;overscroll-behavior:contain;border:0;border-top:var(--rule) solid var(--color-line);border-radius:0 0 var(--radius-card) var(--radius-card);background:var(--color-warm-off-white);box-shadow:0 18px 36px rgb(11 29 51 / .22);padding:.75rem .85rem 1rem;opacity:0;visibility:hidden;pointer-events:none;transform:translateY(calc(-100% - 1rem));transition:transform var(--dur-base) var(--ease-out),opacity var(--dur-fast) var(--ease-out),visibility var(--dur-fast) var(--ease-out)}
  .header-nav-panel .site-nav.is-open{opacity:1;visibility:visible;pointer-events:auto;transform:translateY(0)}
  .header-nav-panel .site-nav a,.header-nav-panel .site-nav .nav-parent,.header-nav-panel .site-nav .nav-cta{width:100%;min-height:2.85rem;justify-content:center;margin:0;border:var(--rule) solid var(--color-line);border-radius:.7rem;background:var(--color-surface);color:var(--color-deep-navy);padding:.7rem .85rem;font-size:var(--text-sm);line-height:1.15;text-align:center;text-transform:none;box-shadow:none}
  .header-nav-panel .site-nav a::before,.header-nav-panel .site-nav a::after,.header-nav-panel .site-nav .nav-parent::after,.header-nav-panel .site-nav .nav-cta::after{content:none;display:none}
  .header-nav-panel .site-nav .nav-dropdown{display:block;width:100%}
  .header-nav-panel .site-nav .nav-menu{display:none}
  .header-nav-panel .site-nav .nav-cta{border-color:transparent;background:var(--color-accent);color:var(--color-white);font-weight:900}
}
@media (max-width:42rem){
  .header-main{min-height:4.7rem;padding:.5rem .65rem}
  .brand-location{display:none}
  .brand-wordmark{height:2.85rem;max-width:10.75rem}
  .header-nav-panel{padding-left:.45rem}
  .nav-toggle{display:inline-flex!important;position:fixed;top:.95rem;right:.75rem;z-index:120;width:2.65rem;min-width:2.65rem;min-height:2.65rem;border:0;background:var(--color-deep-navy)!important;box-shadow:0 12px 26px rgb(11 29 51 / .22)}
  .nav-toggle span{background:var(--color-white)}
}`;

write("tokens.css", tokensCss);
write("styles.css", stylesCss + stickyCtaCss + heroSplitCss + mobileHeroCss + imageCardCss + homepageCurrentCss + headerSplitCss + headerLogoCss + headerLogoFitCss + headerSlashCss + headerVisibleSlashCss + headerColorPanelCss + cityProofCardCss + serviceLinkCardCss + accessibilityColorCss + headerStabilizeCss);

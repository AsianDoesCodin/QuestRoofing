...
const cacheVersion = "2026-05-31-hero-header-cache-01";
...
function footer(prefix = "") {
  ...
  return `<footer class="site-footer">
    <div class="container footer-layout">
      <div><div class="footer-brand-lockup"><span class="brand-mark footer-brand-mark"><img src="${asset(prefix, logoPath)}" alt="Quest Roofing" width="1184" height="624"></span><p class="footer-brand">Quest Roofing</p></div><p>Queen Creek, Arizona roofing contractor serving the Greater Phoenix area with photo-backed inspections and written estimates.</p><ul class="footer-contact"><li><span>Phone</span><a href="${phoneHref}">${phone}</a></li><li><span>Email</span><a href="mailto:${email}">${email}</a></li><li><span>License</span>${roc()}</li><li><span>Credential</span>${gaf()}</li><li><span>Trust</span><strong>Licensed, Bonded, Insured</strong></li></ul></div>
...
function homepage() {
  ...
  const body = `<main id="main">
    ...
    ${section("What roof issue are you dealing with?", "Choose the closest concern. Quest can inspect, document, and explain the right repair or replacement path.", `<div class="issue-grid">${issueCards.map(([h, p, photo]) => `<article class="issue-card image-card">${mediaFrame(photo, "", " loading=\"lazy\" decoding=\"async\"")}<div class="image-card-body"><h3>${h}</h3><p>${p}</p><a href="#estimate">Request inspection <span aria-hidden="true">&rarr;</span></a></div></article>`).join("")}</div>`, "section-issues")}
    ...
    ${section("Roofing services for Arizona homes.", "From urgent leaks to full roof replacement, Quest handles the roof systems common across Queen Creek and the Greater Phoenix area.", `<div class="service-card-grid">${services.map((service) => `<article class="service-card image-card">${mediaFrame(service.image, "", " loading=\"lazy\" decoding=\"async\"")}<div class="image-card-body"><h3>${service.nav}</h3><p>${service.intro}</p><a href="services/${service.slug}/index.html">View service <span aria-hidden="true">&rarr;</span></a></div></article>`).join("")}</div>`, "", "services")}
...
write("styles.css", stylesCss + stickyCtaCss + heroSplitCss + mobileHeroCss + imageCardCss + headerSplitCss);

...
const cacheVersion = "2026-06-02-service-summary-cards-08";
...
const serviceSummaries = {
  "roof-repair": ["RR", "Repair scope", "Leaks, ceiling stains, flashing issues, slipped tile, lifted shingles, and foam cracks."],
  "tile-roofing": ["TR", "Tile systems", "Tile, underlayment clues, battens, valleys, flashing, and finished roof details."],
  "shingle-roofing": ["SR", "Shingle systems", "Surface wear, missing tabs, flashing, ventilation details, and replacement planning."],
  "metal-roofing": ["MR", "Metal details", "Seams, fasteners, trim, valleys, penetrations, and weather-exposed details."],
  "foam-roofing": ["FR", "Flat and foam", "Coating wear, cracks, ponding, punctures, drainage, and waterproofing concerns."],
  "roof-inspection": ["FI", "Free inspection", "Photos, condition notes, repair or replacement context, and written next steps."]
};

const serviceSummaryCard = (service) => {
  const [abbr, kicker, text] = serviceSummaries[service.slug];
  return `<article class="service-card service-card--summary"><span class="service-card-icon" aria-hidden="true">${abbr}</span><div class="service-card-body"><p class="service-card-kicker">${kicker}</p><h3>${service.nav}</h3><p>${text}</p><a href="services/${service.slug}/index.html">View service <span aria-hidden="true">&rarr;</span></a></div></article>`;
};
    ...
    <div class="header-main">
      ...
      <div class="header-nav-panel">
        ...
        <nav class="site-nav" id="site-nav" aria-label="Primary navigation">
          <a href="${home(prefix, "services")}">Services</a>
...
function homepage() {
  ...
  const body = `<main id="main">
    ...
    ${section("Roofing services for Arizona homes.", "From urgent leaks to full roof replacement, Quest handles the roof systems common across Queen Creek and the Greater Phoenix area.", `<div class="service-card-grid service-card-grid--summary">${services.map(serviceSummaryCard).join("")}</div>`, "", "services")}
...
write("styles.css", stylesCss + stickyCtaCss + heroSplitCss + mobileHeroCss + imageCardCss + headerSplitCss + headerLogoCss + headerLogoFitCss + headerSlashCss + headerVisibleSlashCss + headerColorPanelCss);
